...
public class CartItemsService {
	...
	public CartItems addItemToCart(Long userId, int packageId, Date startDate2, Integer noOfPersons, Boolean insurance,
			Double price) {
		try {
			...
			if (userOptional.isPresent() && packageOptional.isPresent()) {
				...
				try {
					...
					// Handle the parsing exception appropriately (e.g., log, throw custom
					// exception)
					logger.error("Error parsing date: {}", startDate2);
					return null;
				}

				// CartItems cartItem = new CartItems(user, package1, startDate, noOfPersons,
				// insurance, price);
	...
	public boolean updateInsurance(int cartItemId, Boolean hasInsurance) {
		try {
			...
			if (optionalCartItem.isPresent()) {
				...
				// Recalculate the price based on insurance (you'll need your pricing logic
				// here)
